...
// A flag is the author's own note that something here is not settled — an
// answer inferred rather than read, a target that needs confirming. It blocks
// publishing, because the whole point of raising it is that a person should
// look before a class does. Clearing it is a deliberate act, not a default.
const F = (where, msg, path) => ({ level: 'flag', where, msg, fix: 'Confirm it, then clear the flag.', path })
...
function auditActivity(a, at, videoIds) {
  ...
  if (String(a.flag || '').trim()) out.push(F(where, a.flag))
  for (const [i, it] of (a.items || []).entries()) {
    if (it && String(it.flag || '').trim()) out.push(F(`${where} \u00b7 item ${i + 1}`, it.flag))
  }
  for (const [i, q] of (a.questions || []).entries()) {
    if (q && String(q.flag || '').trim()) out.push(F(`${where} \u00b7 question ${i + 1}`, q.flag))
  }

  if (a.kind === 'hunt' || a.kind === 'choose') {
...
export function pointsOf(ws) {
  return (ws.activities || []).reduce((n, a) =>
    n + (a.kind === 'hunt' || a.kind === 'choose' ? auditHunt(a.text).errors.length
...
function auditWorksheet(ws, label, videoIds, state) {
  ...
  if (String(ws.flag || '').trim()) out.push(F(label, ws.flag))
...
export function proofPath(topic, opts = {}) {
  ...
  const flags = out.filter((p) => p.level === 'flag').length
  const warnings = out.length - errors - flags
  // A flag stops a publish the same way an error does. The difference is who
  // resolves it: an error is a defect, a flag is a question for a person.
  return { problems: out, errors, warnings, flags, points, stops: core.length + (topic.full ? 1 : 0), ok: errors === 0 && flags === 0 }
}

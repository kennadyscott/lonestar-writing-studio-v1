...
function auditWorksheet(ws, label, videoIds) {
  const out = []
...
export function proofPath(topic, opts = {}) {
  ...
  if (!String(topic.standards || '').trim()) out.push(W('Path', 'No standards listed.', 'Teachers filter by TEKS.'))
  if (!topic.grade) out.push(W('Path', 'No grade level.'))
  ...
  core.forEach((ws, i) => {
    ...
    for (const p of auditWorksheet(ws, label, videoIds)) out.push(p)
    ...
    else for (const p of auditWorksheet(sb, `${label} → Skill Builder`, videoIds)) out.push(p)
  })

  if (!topic.full) out.push(E('Full Topic', 'No capstone.', 'The path has no finish line.'))
  else for (const p of auditWorksheet(topic.full, 'Full Topic', videoIds)) out.push(p)

...
export async function libraryRoute({ method, pathname, body = {}, query = {}, authorized }) {
  ...
  if (method === 'POST' && seg[2] === 'draft') {
    ...
    if (!key) return bad('No ANTHROPIC_API_KEY is set on this host, so activities cannot be drafted here.')
    ...
    if (!r.ok) return { status: 502, json: { error: `Anthropic ${r.status}: ${(await r.text()).slice(0, 200)}` } }

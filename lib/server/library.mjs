...
export async function libraryRoute({ method, pathname, body = {}, authorized }) {
  ...
  if (method === 'GET' && seg[0] === 'meta') {
    return ok({ backend, videos: videoIds(), passcode: !!process.env.PUBLISHER_PASSCODE })
  }
  ...
  if (method === 'POST' && !id) {
    ...
    const draft = {
      ...
      grade: topic.grade ?? null, standards: topic.standards || '', blurb: topic.blurb || '', icon: topic.icon || '📘',
      core: topic.core || [], skillBuilders: topic.skillBuilders || {}, full: topic.full || null,
    }
    const row = await store.putPath({ id: newId, title: draft.title, short: draft.short, grade: draft.grade, standards: draft.standards, blurb: draft.blurb, icon: draft.icon, status: 'draft', live_version: null, draft })
  ...
  if (method === 'PUT' && !seg[2]) {
    ...
    const saved = await store.putPath({
      id, title: draft.title, short: draft.short, grade: draft.grade, standards: draft.standards,
      blurb: draft.blurb, icon: draft.icon, draft,
...
async function seed(body) {
  const { rawTopics } = await import('../../server/proofRoom.mjs')
  ...
  for (const t of topics) {
    ...
    await store.putPath({
      ...
      blurb: t.blurb, icon: t.icon, status: existing?.status || 'draft',

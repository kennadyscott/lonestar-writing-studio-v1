...
export async function libraryRoute({ method, pathname, body = {}, authorized }) {
  ...
  if (method === 'PUT' && !seg[2]) {
    ...
    const saved = await store.putPath({
      ...
      domain: draft.domain, standards: draft.standards, blurb: draft.blurb, icon: draft.icon, draft,
...
async function seed(body) {
  ...
  for (const t of topics) {
    ...
    await store.putPath({
      ...
      standards: t.standards, blurb: t.blurb, icon: t.icon, status: existing?.status || 'draft',

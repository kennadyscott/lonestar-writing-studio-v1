...
import { store, backend, coreUpsert, coreCount, coreSelect } from './store.mjs'
...
import { TEKS_TX } from '../../data/teks-tx.mjs'
...
export async function libraryRoute({ method, pathname, body = {}, query = {}, authorized }) {
  ...
  if (method === 'GET' && seg[0] === 'meta') {
    ...
    return ok({
      ...
      shipped: rawTopics().map((t) => t.id),
      catalog, catalogAvailable: TEKS_TX.length,
    })
  }

  // The shared standards catalog. Content tags against it; nothing writes it
  // except an import, because its authority comes from being verbatim TEA text.
  // One click, no terminal: the catalog ships inside the function, so importing
  // it is a button rather than a command someone has to hold a key for.
  if (method === 'POST' && seg[0] === 'core' && seg[1] === 'import') {
    const clean = TEKS_TX.map((r) => ({
      state: r.state || 'TX', subject: r.subject, grade: String(r.grade),
      course: r.course || 'Grade Level', adopted: r.adopted || null,
      domain: r.domain || '', standard_id: r.standard_id,
      standard_name: r.standard_name || '', description: r.description || '',
      parent_id: r.parent_id || null, source_url: r.source_url || null, verified: true,
    }))
    const n = await coreUpsert('standards', clean, 'state,subject,grade,course,standard_id')
    return ok({ imported: n, count: await coreCount('standards') })
  }

  if (seg[0] === 'core' && seg[1] === 'standards') {
    if (method === 'GET') {
      const p = body.query || {}
    ...
    if (method === 'POST') {
      ...
      if (!Array.isArray(rows) || !rows.length) return bad('No rows sent.')
      const clean = rows.map((r) => ({
        state: r.state || 'TX', subject: r.subject, grade: String(r.grade),
        course: r.course || 'Grade Level', adopted: r.adopted || null,
        domain: r.domain || '', standard_id: r.standard_id,
        standard_name: r.standard_name || '', description: r.description || '',
        parent_id: r.parent_id || null, source_url: r.source_url || null,

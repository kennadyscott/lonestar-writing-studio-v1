...
import { store, backend, coreUpsert, coreCount } from './store.mjs'
...
export async function libraryRoute({ method, pathname, body = {}, authorized }) {
  ...
  if (method === 'POST' && seg[0] === 'core' && seg[1] === 'import') {
    ...
    const n = await coreUpsert('standards', clean, 'state,subject,grade,standard_id')
    return ok({ imported: n, count: await coreCount('standards') })
  }

  if (seg[0] === 'core' && seg[1] === 'standards') {
    if (method === 'GET') {
      return ok({ count: await coreCount('standards'), backend })
    }
    if (method === 'POST') {

...
/* Upsert into the shared core schema. PostgREST reaches a non-public schema
 * through the Accept-Profile / Content-Profile headers rather than a prefixed
 * table name. */
export async function coreUpsert(table, rows, onConflict) {
  if (backend !== 'supabase') throw new Error('The core catalog needs SUPABASE_URL — it is not part of the local file store.')
  const out = []
  for (let i = 0; i < rows.length; i += 500) {
    ...
    const r = await fetch(`${URL_}/rest/v1/${table}?on_conflict=${onConflict}`, {
      ...
      headers: {
        ...
        'content-profile': 'core', 'accept-profile': 'core',
        prefer: 'resolution=merge-duplicates,return=minimal',
      },
      body: JSON.stringify(chunk),
    })
    if (!r.ok) throw new Error(`core upsert ${r.status}: ${(await r.text()).slice(0, 300)}`)
    out.push(chunk.length)
  }
  return out.reduce((a, b) => a + b, 0)
}

/* Read from the shared schema with the filters the browser actually uses. */
export async function coreSelect(table, { where = {}, search = '', limit = 100, offset = 0, order = 'standard_id' } = {}) {
  if (backend !== 'supabase') return { rows: [], total: 0 }
  const qs = ['select=*', `order=${order}`, `limit=${limit}`, `offset=${offset}`]
  for (const [k, v] of Object.entries(where)) if (v) qs.push(`${k}=eq.${encodeURIComponent(v)}`)
  if (search) {
    const t = encodeURIComponent(`*${search}*`)
    qs.push(`or=(standard_id.ilike.${t},standard_name.ilike.${t},description.ilike.${t})`)
  }
  const r = await fetch(`${URL_}/rest/v1/${table}?${qs.join('&')}`, {
    headers: { apikey: KEY, authorization: `Bearer ${KEY}`, 'accept-profile': 'core', prefer: 'count=exact' },
  })
  if (!r.ok) throw new Error(`core select ${r.status}: ${(await r.text()).slice(0, 200)}`)
  const rows = await r.json()
  const total = Number((r.headers.get('content-range') || '/0').split('/')[1] || 0)
  return { rows, total }
}

export async function coreCount(table) {
  if (backend !== 'supabase') return 0
  const r = await fetch(`${URL_}/rest/v1/${table}?select=id`, {
    headers: { apikey: KEY, authorization: `Bearer ${KEY}`, 'accept-profile': 'core', prefer: 'count=exact', range: '0-0' },
  })
...
const supabaseStore = {
  async listPaths() {
    return pg('paths?select=id,title,short,state,grade,domain,standards,blurb,icon,status,live_version,updated_at&order=updated_at.desc')
  },
  async getPath(id) {

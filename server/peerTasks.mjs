...
// Daily Revision Challenge bank — Pip's rough drafts students judge and rewrite.
//
...
/* The grader's own read of Pip's draft — the step-1 answer key. */
...
export const PEER_TASKS = [
  /* ===== ARGUMENT ===== */
  { id: 'pr_recess', author: 'Pip', genre: 'argument', bands: {
    elem: {
      prompt: 'Pip thinks recess should be longer, but only says it is fun. Help Pip give a real reason and finish with a strong ending.',
    ...
    mid: {
      prompt: 'Pip wrote three paragraphs arguing for longer recess, but the reasons have no evidence and Pip never answers the other side. Evaluate it against the rubric, then revise it.',
  ...
  { id: 'pr_uniform', author: 'Pip', genre: 'argument', bands: {
    elem: {
      prompt: 'Pip wrote about school uniforms but repeated the same word over and over. Give Pip a real reason and a closing sentence.',
    ...
    mid: {
      prompt: 'Pip argues against school uniforms, but every reason is an opinion with nothing behind it and Pip ignores what the other side thinks.',
  ...
  { id: 'pr_homework', author: 'Pip', genre: 'argument', bands: {
    elem: {
      prompt: 'Pip says homework should be optional but never says why. Add a real because-reason and a closing sentence.',
    ...
    mid: {
      prompt: 'Pip argues homework should be optional. The claim is there, but there is no evidence, no counter-argument, and a conclusion that just repeats the claim.',
  ...
  { id: 'pr_phones', author: 'Pip', genre: 'argument', bands: {
    elem: {
      prompt: 'Pip wrote about phones at school. Help Pip state the opinion clearly, give a real reason, and end it well.',
    ...
    mid: {
      prompt: 'Pip wants phones allowed at lunch. The reasons are vague, Pip never names what teachers would say back, and the paragraphs run together.',
  ...
  { id: 'pr_yearround', author: 'Pip', genre: 'argument', bands: {
    elem: {
      prompt: 'Pip does not want year-round school but only says it is boring. Give Pip a real reason and a strong ending.',
    ...
    mid: {
      prompt: 'Pip argues against year-round school. The claim is there, but nothing backs it up and the other side never gets a hearing.',
  ...
  { id: 'pr_classpet', author: 'Pip', genre: 'argument', bands: {
    elem: {
      prompt: 'Pip wants a class pet. Help Pip give a real reason instead of just saying it would be fun.',
      weakText: 'We should get a class pet. A pet would be fun. Pets are fun to have. It would be so fun.',
      checklist: argElem([' fun', ' cool', ' nice']) },
    mid: {
      prompt: 'Pip argues for a class pet, but never explains who would care for it, gives no examples, and skips the obvious objection.',
  ...
  { id: 'pr_recycle', author: 'Pip', genre: 'argument', bands: {
    elem: {
      prompt: 'Pip wants recycling bins in every classroom. Help Pip give a real reason and end with a request.',
    ...
    mid: {
      prompt: 'Pip argues for classroom recycling bins. The reasons are general, there are no numbers or examples, and nobody answers the cost question.',
  ...
  { id: 'pr_starttime', author: 'Pip', genre: 'argument', bands: {
    elem: {
      prompt: 'Pip wants school to start later. Help Pip say why with a real because-reason.',
      weakText: 'School should start later. I am tired. Mornings are hard. Later would be better.',
      checklist: argElem([' good', ' bad', ' stuff']) },
    mid: {
      prompt: 'Pip argues for a later start time. There is real science on this, but the draft has none of it — and no answer for parents who work early.',
  ...
  /* ===== INFORMATIONAL ===== */
  { id: 'pr_desert', author: 'Pip', genre: 'informational', bands: {
    elem: {
      prompt: 'Pip wrote about deserts but only says they are dry. Add two real facts, explain them, and finish with an ending sentence.',
    ...
    mid: {
      prompt: "Pip wrote a three-paragraph explanation of deserts, but it's bare. Evaluate it against the rubric, then revise with organized facts, real examples, and precise vocabulary.",
  ...
  { id: 'pr_volcano', author: 'Pip', genre: 'informational', bands: {
    elem: {
      prompt: 'Pip wrote about volcanoes but forgot to explain anything. Add facts and details that tell how a volcano works.',
    ...
    mid: {
      prompt: 'Pip explains volcanoes, but the facts are scattered, nothing is explained, and the vocabulary is fuzzy.',
  ...
  { id: 'pr_bees', author: 'Pip', genre: 'informational', bands: {
    elem: {
      prompt: 'Pip wrote about honeybees. Help Pip add two facts and explain why bees matter.',
    ...
    mid: {
      prompt: 'Pip explains why honeybees matter, but the draft lists facts without connecting them and never uses the words a science teacher would expect.',
  ...
  { id: 'pr_watercycle', author: 'Pip', genre: 'informational', bands: {
    elem: {
      prompt: 'Pip wrote about the water cycle but skipped the steps. Add the steps and explain them.',
    ...
    mid: {
      prompt: 'Pip explains the water cycle, but the steps are out of order, nothing is explained, and the science words are missing.',
  ...
  { id: 'pr_sharks', author: 'Pip', genre: 'informational', bands: {
    elem: {
      prompt: 'Pip wrote about sharks but only said they are scary. Add real facts and explain them.',
    ...
    mid: {
      prompt: 'Pip explains sharks, but the draft repeats itself, contradicts itself, and never explains a single fact.',
  ...
  { id: 'pr_egypt', author: 'Pip', genre: 'informational', bands: {
    elem: {
      prompt: 'Pip wrote about ancient Egypt but forgot the details. Add two facts and explain them.',
    ...
    mid: {
      prompt: 'Pip explains ancient Egypt, but the facts jump around and nothing gets explained or exemplified.',
  ...
  { id: 'pr_hurricane', author: 'Pip', genre: 'informational', bands: {
    elem: {
      prompt: 'Pip wrote about hurricanes but only said they are windy. Add facts and explain how people stay safe.',
    ...
    mid: {
      prompt: 'Pip explains hurricanes. The paragraphs are unsorted, the vocabulary is everyday instead of scientific, and filler sentences pad it out.',
  ...
  { id: 'pr_solar', author: 'Pip', genre: 'informational', bands: {
    elem: {
      prompt: 'Pip wrote about solar power but did not explain how it works. Add facts and details.',
    ...
    mid: {
      prompt: 'Pip explains solar power, but the facts sit in one lump, nothing is explained with an example, and the technical words are missing.',
  ...
  /* ===== NARRATIVE ===== */
  { id: 'pr_lostdog', author: 'Pip', genre: 'narrative', bands: {
    elem: {
      prompt: 'Pip wrote about losing a dog but just tells you how it felt. Show the feelings and give it a real ending.',
    ...
    mid: {
      prompt: 'Pip tells about losing a dog, but the story opens flat, names every feeling instead of showing it, and just stops at the end.',
  ...
  { id: 'pr_newschool', author: 'Pip', genre: 'narrative', bands: {
    elem: {
      prompt: 'Pip wrote about a first day at a new school. Help Pip show the feelings and give a real ending.',
    ...
    mid: {
      prompt: 'Pip writes about a first day at a new school. The opening is a stock line, feelings are labeled, and every sentence starts the same way.',
  ...
  { id: 'pr_storm', author: 'Pip', genre: 'narrative', bands: {
    elem: {
      prompt: 'Pip wrote about a storm at a campsite. Add describing words and show how it felt.',
    ...
    mid: {
      prompt: 'Pip tells about a storm on a camping trip. There is no sound or feeling in it, and the ending simply stops.',
  ...
  { id: 'pr_game', author: 'Pip', genre: 'narrative', bands: {
    elem: {
      prompt: 'Pip wrote about the last play of a game. Show the feelings instead of naming them, and give it a real ending.',
    ...
    mid: {
      prompt: 'Pip writes about the last play of a game. The feelings are labeled, sentences all start alike, and nothing connects one event to the next.',
  ...
  { id: 'pr_attic', author: 'Pip', genre: 'narrative', bands: {
    elem: {
      prompt: 'Pip found a locked door in the attic. Add describing words and finish the story.',
    ...
    mid: {
      prompt: 'Pip tells about a locked attic door, but the opening is flat, nothing is described, and the story ends without resolving.',
  ...
  { id: 'pr_fair', author: 'Pip', genre: 'narrative', bands: {
    elem: {
      prompt: 'Pip got lost at the fair. Show how it felt and give the story a real ending.',
    ...
    mid: {
      prompt: 'Pip writes about getting lost at the fair. The scene has no sights or sounds, feelings are labeled, and the ending just stops.',
  ...
  { id: 'pr_robotgarden', author: 'Pip', genre: 'narrative', bands: {
    elem: {
      prompt: 'Pip wrote about a robot waking up in a garden. Add describing words and finish the story.',
    ...
    mid: {
      prompt: 'Pip writes about a robot waking up in a garden. Good idea, but nothing is described, and every sentence starts the same way.',
  ...
  { id: 'pr_snowday', author: 'Pip', genre: 'narrative', bands: {
    elem: {
      prompt: 'Pip wrote about a snow day. Add describing words and show how it felt.',
    ...
    mid: {
      prompt: 'Pip writes about a snow day. Every feeling is named instead of shown and the events never build on each other.',
  ...
  { id: 'pr_bird', author: 'Pip', genre: 'narrative', bands: {
    elem: {
      prompt: 'Pip found a hurt bird. Show the feelings and give the story a real ending.',
    ...
    mid: {
      prompt: 'Pip writes about rescuing a hurt bird. The opening is generic, feelings are told not shown, and the ending stops instead of landing.',

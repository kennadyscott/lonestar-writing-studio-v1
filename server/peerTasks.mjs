...
export const PEER_TASKS = [
  /* ===== ARGUMENT ===== */
  { id: 'pr_recess', author: 'Blip', genre: 'argument', bands: {
    elem: {
      prompt: 'Blip thinks recess should be longer, but only says it is fun. Help Blip give a real reason and finish with a strong ending.',
    ...
    mid: {
      prompt: 'Blip wrote three paragraphs arguing for longer recess, but the reasons have no evidence and Blip never answers the other side. Evaluate it against the rubric, then revise it.',
  ...
  { id: 'pr_uniform', author: 'Blip', genre: 'argument', bands: {
    elem: {
      prompt: 'Blip wrote about school uniforms but repeated the same word over and over. Give Blip a real reason and a closing sentence.',
    ...
    mid: {
      prompt: 'Blip argues against school uniforms, but every reason is an opinion with nothing behind it and Blip ignores what the other side thinks.',
  ...
  { id: 'pr_homework', author: 'Blip', genre: 'argument', bands: {
    elem: {
      prompt: 'Blip says homework should be optional but never says why. Add a real because-reason and a closing sentence.',
    ...
    mid: {
      prompt: 'Blip argues homework should be optional. The claim is there, but there is no evidence, no counter-argument, and a conclusion that just repeats the claim.',
  ...
  { id: 'pr_phones', author: 'Blip', genre: 'argument', bands: {
    elem: {
      prompt: 'Blip wrote about phones at school. Help Blip state the opinion clearly, give a real reason, and end it well.',
    ...
    mid: {
      prompt: 'Blip wants phones allowed at lunch. The reasons are vague, Blip never names what teachers would say back, and the paragraphs run together.',
  ...
  { id: 'pr_yearround', author: 'Blip', genre: 'argument', bands: {
    elem: {
      prompt: 'Blip does not want year-round school but only says it is boring. Give Blip a real reason and a strong ending.',
    ...
    mid: {
      prompt: 'Blip argues against year-round school. The claim is there, but nothing backs it up and the other side never gets a hearing.',
  ...
  { id: 'pr_classpet', author: 'Blip', genre: 'argument', bands: {
    elem: {
      prompt: 'Blip wants a class pet. Help Blip give a real reason instead of just saying it would be fun.',
      weakText: 'We should get a class pet. A pet would be fun. Pets are fun to have. It would be so fun.',
      checklist: argElem([' fun', ' cool', ' nice']) },
    mid: {
      prompt: 'Blip argues for a class pet, but never explains who would care for it, gives no examples, and skips the obvious objection.',
  ...
  { id: 'pr_recycle', author: 'Blip', genre: 'argument', bands: {
    elem: {
      prompt: 'Blip wants recycling bins in every classroom. Help Blip give a real reason and end with a request.',
    ...
    mid: {
      prompt: 'Blip argues for classroom recycling bins. The reasons are general, there are no numbers or examples, and nobody answers the cost question.',
  ...
  { id: 'pr_starttime', author: 'Blip', genre: 'argument', bands: {
    elem: {
      prompt: 'Blip wants school to start later. Help Blip say why with a real because-reason.',
      weakText: 'School should start later. I am tired. Mornings are hard. Later would be better.',
      checklist: argElem([' good', ' bad', ' stuff']) },
    mid: {
      prompt: 'Blip argues for a later start time. There is real science on this, but the draft has none of it — and no answer for parents who work early.',
  ...
  /* ===== INFORMATIONAL ===== */
  { id: 'pr_desert', author: 'Blip', genre: 'informational', bands: {
    elem: {
      prompt: 'Blip wrote about deserts but only says they are dry. Add two real facts, explain them, and finish with an ending sentence.',
    ...
    mid: {
      prompt: "Blip wrote a three-paragraph explanation of deserts, but it's bare. Evaluate it against the rubric, then revise with organized facts, real examples, and precise vocabulary.",
  ...
  { id: 'pr_volcano', author: 'Blip', genre: 'informational', bands: {
    elem: {
      prompt: 'Blip wrote about volcanoes but forgot to explain anything. Add facts and details that tell how a volcano works.',
    ...
    mid: {
      prompt: 'Blip explains volcanoes, but the facts are scattered, nothing is explained, and the vocabulary is fuzzy.',
  ...
  { id: 'pr_bees', author: 'Blip', genre: 'informational', bands: {
    elem: {
      prompt: 'Blip wrote about honeybees. Help Blip add two facts and explain why bees matter.',
    ...
    mid: {
      prompt: 'Blip explains why honeybees matter, but the draft lists facts without connecting them and never uses the words a science teacher would expect.',
  ...
  { id: 'pr_watercycle', author: 'Blip', genre: 'informational', bands: {
    elem: {
      prompt: 'Blip wrote about the water cycle but skipped the steps. Add the steps and explain them.',
    ...
    mid: {
      prompt: 'Blip explains the water cycle, but the steps are out of order, nothing is explained, and the science words are missing.',
  ...
  { id: 'pr_sharks', author: 'Blip', genre: 'informational', bands: {
    elem: {
      prompt: 'Blip wrote about sharks but only said they are scary. Add real facts and explain them.',
    ...
    mid: {
      prompt: 'Blip explains sharks, but the draft repeats itself, contradicts itself, and never explains a single fact.',
  ...
  { id: 'pr_egypt', author: 'Blip', genre: 'informational', bands: {
    elem: {
      prompt: 'Blip wrote about ancient Egypt but forgot the details. Add two facts and explain them.',
    ...
    mid: {
      prompt: 'Blip explains ancient Egypt, but the facts jump around and nothing gets explained or exemplified.',
  ...
  { id: 'pr_hurricane', author: 'Blip', genre: 'informational', bands: {
    elem: {
      prompt: 'Blip wrote about hurricanes but only said they are windy. Add facts and explain how people stay safe.',
    ...
    mid: {
      prompt: 'Blip explains hurricanes. The paragraphs are unsorted, the vocabulary is everyday instead of scientific, and filler sentences pad it out.',
  ...
  { id: 'pr_solar', author: 'Blip', genre: 'informational', bands: {
    elem: {
      prompt: 'Blip wrote about solar power but did not explain how it works. Add facts and details.',
    ...
    mid: {
      prompt: 'Blip explains solar power, but the facts sit in one lump, nothing is explained with an example, and the technical words are missing.',
  ...
  /* ===== NARRATIVE ===== */
  { id: 'pr_lostdog', author: 'Blip', genre: 'narrative', bands: {
    elem: {
      prompt: 'Blip wrote about losing a dog but just tells you how it felt. Show the feelings and give it a real ending.',
    ...
    mid: {
      prompt: 'Blip tells about losing a dog, but the story opens flat, names every feeling instead of showing it, and just stops at the end.',
  ...
  { id: 'pr_newschool', author: 'Blip', genre: 'narrative', bands: {
    elem: {
      prompt: 'Blip wrote about a first day at a new school. Help Blip show the feelings and give a real ending.',
    ...
    mid: {
      prompt: 'Blip writes about a first day at a new school. The opening is a stock line, feelings are labeled, and every sentence starts the same way.',
  ...
  { id: 'pr_storm', author: 'Blip', genre: 'narrative', bands: {
    elem: {
  ...
  { id: 'pr_game', author: 'Blip', genre: 'narrative', bands: {
    elem: {
      prompt: 'Blip wrote about the last play of a game. Show the feelings instead of naming them, and give it a real ending.',
    ...
    mid: {
      prompt: 'Blip writes about the last play of a game. The feelings are labeled, sentences all start alike, and nothing connects one event to the next.',
  ...
  { id: 'pr_attic', author: 'Blip', genre: 'narrative', bands: {
    elem: {
      prompt: 'Blip found a locked door in the attic. Add describing words and finish the story.',
    ...
    mid: {
      prompt: 'Blip tells about a locked attic door, but the opening is flat, nothing is described, and the story ends without resolving.',
  ...
  { id: 'pr_fair', author: 'Blip', genre: 'narrative', bands: {
    elem: {
      prompt: 'Blip got lost at the fair. Show how it felt and give the story a real ending.',
    ...
    mid: {
      prompt: 'Blip writes about getting lost at the fair. The scene has no sights or sounds, feelings are labeled, and the ending just stops.',
  ...
  { id: 'pr_robotgarden', author: 'Blip', genre: 'narrative', bands: {
    elem: {
      prompt: 'Blip wrote about a robot waking up in a garden. Add describing words and finish the story.',
    ...
    mid: {
      prompt: 'Blip writes about a robot waking up in a garden. Good idea, but nothing is described, and every sentence starts the same way.',
  ...
  { id: 'pr_snowday', author: 'Blip', genre: 'narrative', bands: {
    elem: {
      prompt: 'Blip wrote about a snow day. Add describing words and show how it felt.',
    ...
    mid: {
      prompt: 'Blip writes about a snow day. Every feeling is named instead of shown and the events never build on each other.',
  ...
  { id: 'pr_bird', author: 'Blip', genre: 'narrative', bands: {
    elem: {
      prompt: 'Blip found a hurt bird. Show the feelings and give the story a real ending.',
    ...
    mid: {
      prompt: 'Blip writes about rescuing a hurt bird. The opening is generic, feelings are told not shown, and the ending stops instead of landing.',

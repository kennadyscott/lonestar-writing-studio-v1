...
export const TOPICS = [
  {
    ...
    skillBuilders: {
      ...
      ws_prep: {
        id: 'sb_prep', title: 'SB: Prepositions', standards: ['5.11D(vi)'], skill: 'One wrong preposition at a time',
        activities: [
          fix('Zuri’s story has eight prepositions that do not fit. Click the right one from the bank for each.',
            ['through', 'beside', 'past', 'between', 'near', 'on', 'across', 'behind'], [
              { given: 'Zuri and her team entered the observatory ____ a winding pathway near the coastal bluff.', answer: 'past', video: 'sb-prep-1' },
              { given: 'Their mentor stood ____ the doorway and explained the research guidelines.', answer: 'beside', video: 'sb-prep-2' },
              { given: 'The visitors walked ____ the metal corridor while shining their lanterns.', answer: 'through', video: 'sb-prep-3' },
              { given: 'Zuri noticed small gears ____ the copper rafters.', answer: 'between', video: 'sb-prep-4' },
              { given: 'Later, the team rested ____ a large telescope beside the platform.', answer: 'behind', video: 'sb-prep-5' },
              { given: 'A small clockwork mouse quickly ran ____ the brass track to hide from the visitors.', answer: 'behind', video: 'sb-prep-6' },
              { given: 'The mentor pointed ____ the observatory where a narrow staircase curved toward the dome.', answer: 'across', video: 'sb-prep-7' },
              { given: 'Before sunset, Zuri placed her notebook ____ the workbench and wrote notes about the observatory.', answer: 'on', video: 'sb-prep-8' },
            ],
            'Read the sentence with your word in it and picture it. If you cannot picture it, it is the wrong word.',
            'select'),
        ],
      },
      ws_pron: {
    ...
    full: {
      ...
      activities: [
        passage('Read the draft, then answer the questions. The sentences are numbered.',
          [
            'In the kingdom of Eldermere, a young falconer named Cedric of Thornwick served at Castle Ravenshield.',
            'One morning, a messenger falcon returned carrying a sealed letter from a distant ally.',
            'Cedric bringed the letter to Lady Isolde, who was preparing for an important council meeting.',
            'The message became more urgent between scouts reported unusual movement near the northern border.',
            'The council discussed possible responses.',
            'They decided to gather more information before taking action.',
            'Sir Alaric of Blackmoor volunteered to lead a patrol through the forest roads.',
            'The situation seemed different than previous border disputes because the neighboring kingdom had remained peaceful for many years.',
            'After several days, the patrol returned with news that traveling merchants had been mistaken for soldiers.',
          ],
          [
            { kind: 'pick', sentence: 3, ask: 'Click the irregular verb error in sentence 3, then write the correction.', target: 'bringed', answer: 'brought' },
            { kind: 'pick', sentence: 4, ask: 'Click the error in sentence 4, then write the word it should be.', target: 'between', answer: 'when',
              flag: 'The worksheet calls this "the pronoun error in sentence 4", but sentence 4 contains no pronoun — the error is "between", which should be a joining word like "when". Reworded here to be answerable. Decide whether to fix the paper.' },
            {
              kind: 'write', sentence: '5 and 6',
              ask: 'How would you combine sentences 5 and 6 into one sentence?',
              checks: [
                need('Joins the two ideas', ANY(HAS('therefore'), HAS('consequently'), HAS('so'), HAS('and'), HAS(';'))),
                need('Keeps both ideas', ALL(HAS('council'), HAS('information'))),
                need('Still one sentence', { type: 'maxSentences', n: 1 }),
              ],
              model: 'The council discussed possible responses; therefore, they decided to gather more information before taking action.',
            },
          ],
          'Every one of these is a skill you have already cleared on the path.'),
        passage('A second draft. Same job, numbered sentences.',
          [
            'On Circuit Grove Elementary, a cafeteria robot named WOB-7 was programmed to make pancakes for students every Friday morning.',
            'The robot usually followed instructions perfectly, but one day a software update caused unexpected problems.',
            'WOB-7 ran a breakfast test and accidentally stacked forty-two pancakes on a single tray.',
            'The situation became ridiculouser when the robot began measuring syrup with a garden watering can instead of a dispenser.',
            'Teachers tried to stop the confusion, however WOB-7 insisted that its actions matched the updated instructions.',
            'A student programmer named Zinnia examined the robot’s code and discovered that several commands had been rearranged during the update.',
            'The errors looked different than the original programming because the importanter lines had been deleted.',
            'After carefully restoring the correct sequence, Zinnia restarted the system and watched the robot prepare normal-sized breakfasts.',
          ],
          [
            { kind: 'blank', sentence: 1, ask: 'The word "On" in sentence 1 should be changed to which word?', answer: 'At' },
            { kind: 'pick', sentence: 4, ask: 'Click the adjective error in sentence 4, then write the correct form.', target: 'ridiculouser', answer: 'more ridiculous' },
            {
              kind: 'write', sentence: 5,
              ask: 'What changes should be made to sentence 5? Rewrite it correctly.',
              checks: [
                need('Keeps however', HAS('however')),
                need('Semicolon or period before however', RX('[;.]\\s*however')),
                need('Comma right after however', RX('however\\s*,')),
                need('Keeps both ideas', ALL(HAS('teachers'), HAS('insisted'))),
                need('Still one sentence', { type: 'maxSentences', n: 1 }),
              ],
              model: 'Teachers tried to stop the confusion; however, WOB-7 insisted that its actions matched the updated instructions.',
            },
            {
              kind: 'write', sentence: 7,
              ask: 'What changes need to be made to sentence 7? Rewrite it correctly.',
              checks: [
                need('Fixes the adjective', HAS('more important')),
                need('The wrong form is gone', { type: 'notContains', any: ['importanter'] }),
                need('Keeps the idea', ALL(HAS('errors'), HAS('deleted'))),
                need('Still one sentence', { type: 'maxSentences', n: 1 }),
              ],
              model: 'The errors looked different from the original programming because the more important lines had been deleted.',
            },
          ],
          'Two adjectives, one joining word and one preposition are hiding in here.'),
      ],
    },
  },
]

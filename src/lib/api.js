...
const networkApi = {
  ...
  fluencyFinish: (payload) => fetch(u('/api/fluency/finish'), { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify(payload) }).then(j),
  fluencyReset: () => fetch(u('/api/fluency/reset'), { method: 'POST' }).then(j),

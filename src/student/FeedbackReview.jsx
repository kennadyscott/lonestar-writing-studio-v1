  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
      <div style={{ textAlign: 'right', lineHeight: 1.1 }}>
        <div style={{ fontSize: 15, fontWeight: 800, color: NAVY, letterSpacing: .4 }}>{t(label)}</div>
        {sub && <div style={{ fontSize: 15, fontWeight: 800, color, letterSpacing: .4 }}>{t(sub)}</div>}
      </div>
      <svg viewBox="0 0 110 110" style={{ width: 104, height: 104, flexShrink: 0 }} role="img" aria-label={pct != null ? `${t(label)}: ${pct}%` : t('{label}: {value} of {max}', { label: t(label), value, max })}>
...
export default function FeedbackReview({ state, sub, onBack }) {
  ...
  const rubricRows = strategyKey === 'CER'
    ...
    : [t('Restates the question in the answer'), t('Answers every part of the question'), t('Uses evidence from the text'), t('Explains how the evidence proves the answer')]
    ...
    <div>
      ...
      <div style={{ maxWidth: 940, margin: '0 auto', background: '#fff', borderRadius: 20, overflow: 'hidden', border: '1px solid var(--line)', boxShadow: '0 10px 30px rgba(13,47,85,.12)' }}>
        ...
        <div style={{ background: NAVY, color: '#fff', padding: '14px 20px', display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <b style={{ flex: 1, textAlign: 'center', fontSize: 19, letterSpacing: .3 }}>{t('Your Writing Feedback')}</b>
          <span style={{ width: 26 }} />
        </div>
        ...
        <div style={{ padding: '18px 22px 22px', background: '#fbfdfe' }}>
          <div style={{ textAlign: 'center' }}>
            <span style={{ display: 'inline-block', background: '#eef3f6', color: NAVY, borderRadius: 999, padding: '6px 16px', fontSize: 13.5, fontWeight: 800 }}>
              {count > 1 ? t('Question No: {n} of {total}', { n: q + 1, total: count }) : t('Question No: {n}', { n: q + 1 })}
            </span>
          </div>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'flex-start', marginBottom: 18 }}>
            <Reveal label={t('Question Prompt')}>{questionText(q) || t('No prompt on file for this question.')}</Reveal>
            <Reveal label={t('Your Answer')}>{answer.trim() || t('No response recorded for this question.')}</Reveal>
          ...
          <div style={{ background: '#f4f8fb', borderRadius: 10, padding: '11px 16px', margin: '16px 0 0', textAlign: 'center', fontSize: 13, color: 'var(--muted)', fontWeight: 700 }}>
            {hitCount === anchors.length
              ? t('🏆 Every anchor hit. Bring this same strategy to your next piece.')
              : t('📈 Keep practicing! Review the feedback above and try again to level up your score.')}
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '14px 20px', borderTop: '1px solid var(--line)' }}>
          {q > 0 && (
            <button onClick={() => setQ(q - 1)} style={{ padding: '11px 18px', borderRadius: 11, fontWeight: 800, fontSize: 13.5, color: 'var(--muted)', background: '#eef3f6', cursor: 'pointer' }}>{t('← Back')}</button>
          )}

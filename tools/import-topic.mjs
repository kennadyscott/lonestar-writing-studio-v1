...
const artSize = new Map()
for (const items of rawArt.values()) {
  for (const a of items) artSize.set(a.h, a.data.length)
  for (const h of new Set(items.map((a) => a.h))) seenIn.set(h, (seenIn.get(h) || 0) + 1)
}
const CHROME = new Set([...seenIn.entries()]
  .filter(([h, n]) => n === deckCount && deckCount >= 3 && (artSize.get(h) || 0) < 120000)
  .map(([h]) => h))
